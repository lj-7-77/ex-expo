...
"""
    ...
    private let descriptionTextView: UITextView = {
        ...
        textView.text = """
                        되는 같은 힘차게 인도하겠다는 그것은 끝까지 부패뿐이다. 바이며, 아니더면, 미인을 대중을 기관과 고동을 이상 인간은 뿐이다. 끓는 광야에서 피가 설산에서 발휘하기 우리의 듣기만 봄바람이다. 역사를 위하여서, 천고에 이상 뿐이다. 사랑의 노년에게서 거친 아름답고 노래하며 지혜는 이것이다. 그들은 얼마나 아니더면, 살 피고 너의 이것이다. 길지 구할 우리 것은 청춘에서만 눈에 속잎나고, 품었기 있다. 실현에 충분히 되는 교향악이다. 능히 같은 대중을 노래하며 청춘의 얼음 말이다. 날카로우나 살았으며, 곳이 아니다.
...
extension IntroView {
    private func setupUI() {
        [titleLabel, posterImageView, visitorsLabel, locationLabel, periodLabel, descriptionTextView].forEach { stackView.addArrangedSubview($0) }
